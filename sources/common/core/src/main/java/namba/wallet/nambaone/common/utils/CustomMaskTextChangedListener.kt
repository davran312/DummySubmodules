...
open class CustomMaskTextChangedListener(
    ...
    fun placeholder(charRepresentation: Char): String {
        var holder = ""
        placeholder().forEach {
            holder += if (it.isLetterOrDigit()) {
                charRepresentation
            } else {
                it
            }
    ...
    companion object {
        ...
        ): CustomMaskTextChangedListener = installOn(
            editText,
            primaryFormat,
            emptyList(),
        ...
        fun installOn(
            ...
            valueListener: ValueListener? = null
        ): CustomMaskTextChangedListener {
            val maskedListener = CustomMaskTextChangedListener(
                primaryFormat,
                affineFormats,
                customNotations,
                affinityCalculationStrategy,
            ...
            editText.setTag(R.id.mask_watcher_tag_id, maskedListener)
            return maskedListener
        }

        fun uninstallFrom(editText: EditText) {
            val maskWatcher = editText.getTag(R.id.mask_watcher_tag_id) as? CustomMaskTextChangedListener
