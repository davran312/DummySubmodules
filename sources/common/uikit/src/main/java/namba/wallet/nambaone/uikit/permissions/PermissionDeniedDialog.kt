...
class PermissionDeniedDialog : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        return AlertDialog.Builder(requireContext(), R.style.AppTheme_AlertDialog)
            .setCancelable(true)
            .setTitle(title ?: getTitle(permission))
            .setMessage(message ?: getText(permission))
            .setPositiveButton(R.string.settings) { _, _ -> openSettings(requireContext()) }
    ...
    private fun getTitle(permission: String) =
        when (permission) {
            ...
            Manifest.permission.WRITE_EXTERNAL_STORAGE -> getString(R.string.storage_permission_alert_title)
    ...
    private fun getText(permission: String) =
        when (permission) {
            ...
            Manifest.permission.READ_EXTERNAL_STORAGE,
            Manifest.permission.WRITE_EXTERNAL_STORAGE -> getString(R.string.storage_permission_alert_message)
            else -> throw IllegalStateException("Unknown permission type $permission")
        }
