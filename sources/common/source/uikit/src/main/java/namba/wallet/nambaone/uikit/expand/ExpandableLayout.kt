package namba.wallet.nambaone.uikit.expand
...
class ExpandableLayout @JvmOverloads constructor(
    ...
    init {
        if (attrs != null) {
            val attr = context.obtainStyledAttributes(attrs, R.styleable.ExpandableLayout)
            duration = attr.getInt(R.styleable.ExpandableLayout_duration, DEFAULT_DURATION)
            val expansion =
                if (attr.getBoolean(R.styleable.ExpandableLayout_expanded, true)) 1f else 0f
            orientation =
                attr.getInt(R.styleable.ExpandableLayout_android_orientation, VERTICAL)
            parallax = attr.getFloat(R.styleable.ExpandableLayout_parallax, 1f)
            attr.recycle()
